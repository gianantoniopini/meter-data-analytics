...
class MeterDataService {
  async getInstantaneousPowerMeasurements(
    ...
    const instantaneousPowerMeasurements = responseBody.data.timeSeries.map((m) =>
      mapToInstantaneousPowerMeasurement(m)
    )

    const instantaneousPowerMeasurementsSorted = instantaneousPowerMeasurements.sort(
      (m1: InstantaneousPowerMeasurement, m2: InstantaneousPowerMeasurement) => {
        return new Date(m1.timestamp).getTime() - new Date(m2.timestamp).getTime()
      }
    )
